...
class DocumentProcessor:
    ...
    def get_pending_cases(self):
        cases = self.redis_client.hgetall("elasticindex:cases")
        return [(case_id) for case_id, status in cases.items() if status == 'P']

    def update_case_status(self, case_id, status):
        self.redis_client.hset("elasticindex:cases", case_id, status)
    ...
    def update_processing_status(self, case_id, bdid, status):
        ...
        pipe = self.redis_client.pipeline()
        key = f"elasticindex:{case_id}:{status}"
        pipe.hset(key, bdid, timestamp)
        pipe.hincrby(f"case:{case_id}:summary", status, 1)
        pipe.execute()
    ...
    def process_case(self, case_id):
        ...
        try:
            ...
            self.redis_client.hset(f"case:{case_id}:summary", "processStart", process_start_time)
            completed_docs = set(self.redis_client.hkeys(f"elasticindex:{case_id}:completed"))
            ...
            def process_document(doc):
                ...
                try:
                    if self.redis_client.hexists(f"elasticindex:{case_id}:completed", doc['bdid']):
            ...
            self.redis_client.hset(f"case:{case_id}:summary", "processEnd", process_end_time)
